...
<form method="POST" action="/blog">
  ...
  <div class="space-y-12 bg-blue-600">
    <div class="border-b border-white/10 pb-12">
      <h2 class="text-base/7 font-semibold text-white">create new post</h2>
      ...
      <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
        <div class="sm:col-span-3">
          ...
          <div class="mt-2">
            <input id="title" value="{{ old('title') }}" type="text" name="title" autocomplete="given-name" class="{{ $errors->has('title') ? 'outline-red-500': 'outline-gray-300' }}  block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1  placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
          </div>
           @error('title')
